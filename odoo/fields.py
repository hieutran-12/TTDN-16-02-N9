...
class Many2one(_Relational):
    ...
    def convert_to_read(self, value, record, use_name_get=True):
        if use_name_get and value:
            ...
            # value can be a proxy (_unknown) coming from the web client during
            # onchange; accessing `.id` may raise AttributeError. Guard it to
            # avoid crashing the whole onchange mechanism. If the id cannot be
            # retrieved, return False so the client receives a safe value.
            try:
                # If value is a simple id (int-like), return it
                from odoo.models import NewId
                if isinstance(value, (int,)):
                    return value
                # Some virtual ids (NewId) are acceptable
                if 'NewId' in globals() and isinstance(value, NewId):
                    return value
                return getattr(value, 'id', False)
            except Exception:
                return False
